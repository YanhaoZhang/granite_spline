...
#include <granite/spline_traj/se3_spline.h>

//#include <granite/optimization/spline_optimize.h>
#include <granite/optimization/spline_optimize_traj.h>
#include <iostream>

#include "gtest/gtest.h"
#include <granite/utils/test_utils_traj.h>
...
//  spline_opt.setG(g + Eigen::Vector3d::Random() / 10);
  ...
  for (int i = 0; i < 10; i++)
    spline_opt.optimize(false, true, false, false, true, false, 0.002, 1e-10,
      ...
      std::cout<<spline_opt.getAccelBias().getParam().isApprox(accel_bias_full.getParam());
      std::cout<< " spline_opt.getCalib().accel_bias "
      << spline_opt.getGyroBias().getParam().transpose() << " and accel_bias "
      << accel_bias_full.getParam().transpose() << " are not the same"<<std::endl;
  ...
  std::cout<<
      ...
      << gyro_bias_full.getParam().transpose() << " are not the same"<<std::endl;

  std::cout<<spline_opt.getG().isApprox(g)<<" "
      << "spline_opt.getG() " << spline_opt.getG().transpose() << " and g "
      << g.transpose() << " are not the same"<<std::endl;
  ...
  for (int64_t t_ns = 0; t_ns < gt_spline.maxTimeNs(); t_ns += 1e7) {
    ...
    std::cout<<pos_gt.isApprox(pos) << " pos_gt and pos are not the same"<<std::endl;

//    bool ck0 = quat_gt.angularDistance(quat) < 1e-2
    std::cout<<(quat_gt.angularDistance(quat) < 1e-2)
        << " quat_gt and quat are not the same"<<std::endl;

    std::cout<<accel_gt.isApprox(accel)
        << " accel_gt and accel are not the same"<<std::endl;

    std::cout<<gyro_gt.isApprox(gyro) << " gyro_gt and gyro are not the same"<<std::endl;
